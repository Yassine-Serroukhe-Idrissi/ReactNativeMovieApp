...
import { getMovieDetails,getMovieImage } from '../API/tmdbapi'
  ...
  render() {
    return (
      <View style={styles.global_container_style}>
        <Image style={styles.image_style} source={{uri:getMovieImage(this.state.details.backdrop_path)}}/>
        ...
        <View>
            ...
            {this.state.details.production_companies?.map((g)=>{return(<Button key={g.id} title={g.name} style={styles.genre_btn}/>)})} 
        </View>
      </View>
    )
  }
